...
class ActuatorController:
            ...
            if HARDWARE_AVAILABLE and not SIM_MODE:
                ...
                for i, addr in enumerate(self.motoron_addresses):
                    try:
                        ...
                        # SAFETY DEPENDENCY (Phase 1, March 16): With hardware timeout disabled,
                        # CommandExecutor._stop_all_motors() is the ONLY safety net for motor 6
                        # during control loss. If that call is ever removed, re-enable this timeout.
                        if i == 3:
                ...
                if self.use_pca9685:
                    ...
                    try:
                        ...
                        # Set to initialization position (60°)
                        ...
                        logger.info(f"PCA9685 servo initialized{mux_info} - channel {self.servo_channel} at 60°")
                    except Exception as e:
    ...
    def stop(self):
        ...
        if self.servo_kit:
            try:
                # Set PCA9685 servo to 60°
                self.servo_kit.servo[self.servo_channel].angle = 60
                logger.info(f"PCA9685 servo set to 60°")
            except Exception as e:
                logger.error(f"Error setting PCA9685 servo to 60°: {e}")
        ...
        with self._lock:
            ...
            if self.servo_kit:
                ...
                try:
                    ...
                    duty_60deg = self.servo_min_duty + 0.3333 * (self.servo_max_duty - self.servo_min_duty)
                    self.servo_pwm.ChangeDutyCycle(duty_60deg)
                    servo_stopped = True
                except Exception as e:
